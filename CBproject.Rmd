...
In this way, information about tumor stiffness could be compared  to RNAsequencing data, as to identify potential markers and features which could explain the observed differences from a molecular point of view, while also taking into account the intratumoral heterogeneity of each patient.
...
The file can be downloaded to be submitted to the pipeline by following instructions on the [Initialization 
script](https://github.com/CBenetti/Data_Analysis_exam/blob/main/Scripts/01_import.R)
...
As we can see, the raw data counts are distributed in 18 columns of a csv file. First column contains versioned Ensembl id of genes,first row contains the sample name. Column names correspond 
...
As to procede into further analysis of the data, the dataeset must be loaded into R environment, annotations should be integrated in a way as to match the original sample set, gene names 
...
Information on how to build, download or use the docker can be found on the [Docker](https://github.com/CBenetti/Data_Analysis_exam/tree/main/Docker) folder on github, and more detailed 
...
according to the architecture in use.
...
Usually, the query of the ahEdb object is done through filtering for de-versioned id. However, since here we dispose of versioned ids, which have the advantage of being univoque, an 
alternative 
...
both as a result of their proximity to crossing over hotspots. By default, in ensembldb such genes are aligned to X chromosome. As a matter of fact, ids without the suffix are already 
present in the geneset and can be ascribed to regions on the X chromosomes.
...
and the replicate identifier, and featureData with gene symbols and gene biotypes.
...
```
With this set of data, it is possible to procede to the analysis on the differential gene expression between samples with different stromal stiffness and evaluate the impact of intratumoral 
...
The expressionset in use for the dimentional reduction refers to normalized and logaritmic form of the data, as per the requirements of the function. Data must be transposed, as we want to 
...
In this case, we cannot get information on how the variance is spread, however we can have a greater insight on how much samples are similar between each other.
...
To further look into this hypotesis, a heatmap is built for a better visualization of the results, comparing differentially expressed genes of stromal stiff and soft samples.
...
those samples seem to be less expressed with respect to the mean of the cells. This could be explained by a higher overall level of stiffnes, in which the cells completely 
...
We see that relationship of mean value of tumor 3 with both tumor 1 and 2 samples is all but linear. Instead, between samples from patient 1 and 2 there is a bit more coherence, with mean 
gene expression values for each gene having a higher degree of similarity. 

Overall, these finding suggest that sample 3 might have different pathways by which stromal stiffness is elicited, while we can work more on the comparisons of patient 1 and 2.
...
To investigate in more depth which genes are directly involved in such annotations, gProfiler offers a schematic view of how the genes are distributed in the enriched gene sets.
...
mechanisms in which stromal composition can affect cancer cell features and overall tumor evolution, providing novel parameters to use durign therapy decision making and prognosis evaluation 
